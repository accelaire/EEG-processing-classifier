# EEG exploratory factor analysis
#
...
def ofr_randperm(feats, labels, threshold):
    ## Init
...
feats = np.concatenate((feats_V2, feats_V3))
print("original features shape: " + str(feats.shape))

labels = np.concatenate((np.zeros(int(feats_V2.shape[0])).T,np.ones(int(feats_V3.shape[0])).T))
